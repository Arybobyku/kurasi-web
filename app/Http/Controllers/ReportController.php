...
class ReportController extends Controller
{
    ...
    public function all(){
        $data= DB::select("SELECT SUM(total_produk) as 'total',transaksi_item.id_product,barang.*,umkm.nama as 'nama_umkm',transaksi_item.created_at AS 'tanggal',ms_kota.nama AS 'kota',ms_kategori.nama AS 'kat' FROM transaksi_item JOIN barang ON barang.kode_produk COLLATE utf8mb4_unicode_ci = transaksi_item.id_product JOIN umkm ON barang.kode_umkm COLLATE utf8mb4_unicode_ci = umkm.kode_umkm JOIN ms_kota  ON ms_kota.kode COLLATE utf8mb4_unicode_ci = barang.kode_kota JOIN ms_kategori ON ms_kategori.kode COLLATE utf8mb4_unicode_ci = barang.kode_kategori GROUP BY transaksi_item.created_at,transaksi_item.id_product ORDER by transaksi_item.created_at DESC, barang.nama ASC");
        //  return response()->json($data);
    ...
    public function kategori(){
        $data= DB::select("SELECT SUM(transaksi_item.total_produk) as 'total',transaksi_item.id_product,ms_kategori.* FROM transaksi_item JOIN barang ON barang.kode_produk COLLATE utf8mb4_unicode_ci = transaksi_item.id_product  JOIN ms_kategori ON ms_kategori.kode COLLATE utf8mb4_unicode_ci = barang.kode_kategori GROUP BY barang.kode_kota ORDER by total DESC, barang.nama ASC");
         return view('report_kota',['data'=>$data]);
    }
    ...
    public function stock_kategori(){
        $data = DB::select("SELECT SUM(barang.stock) AS 'total',ms_kategori.* FROM barang JOIN ms_kategori ON barang.kode_kategori COLLATE utf8mb4_unicode_ci = ms_kategori.kode GROUP BY ms_kategori.kode ORDER by total DESC, barang.nama ASC");
